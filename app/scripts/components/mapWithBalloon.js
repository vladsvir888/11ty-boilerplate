const mapWithBalloon = () => {
  ...
  ymaps.ready(() => {
    ...
    const balloonTemplate = document.getElementById('balloon').content.cloneNode(true).querySelector('.balloon').outerHTML;
    ...
    const baloonLayout = ymaps.templateLayoutFactory.createClass(balloonTemplate, {
      ...
      onChoiceClick(e) {
        const id = e.target.closest('.balloon').id;

        mapNode.dispatchEvent(new CustomEvent('store-choice', {
          detail: { id },
        }));
      },
    });
